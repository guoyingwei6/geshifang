...
工具栏的「选择文章模板」会切换整套文章视觉语言，而不只是标题配色。选择后若编辑区已有内容，会立即重新排版方便对比。

| 模板 | 风格与适用场景 |
|------|----------------|
| 格式坊经典（原默认） | 保留改造前的红橙色默认排版 |
| 摸鱼绿（默认） | 清爽、高信息密度，适合教程、清单与工具盘点 |
| 红白 | 强对比、有力量感，适合深度观点与分析 |
| 摸鱼票据 | 票根/收据视觉，适合工具对比与创意评测 |
| 橄榄手记 | 克制的内刊手记感，适合深度评测与案例复盘 |

内置模板覆盖标题、正文、强调、引用、列表、代码块、表格、图片容器与分割线。点击模板旁的 `+`，仍可将当前颜色、字号、字体等调整保存到「我的模板」。
...
| 元素 | 语法示例 | 说明 |
|------|----------|------|
| H1 | `# 标题` / `## 标题` / `一、标题` | 居中红橙色章节标题 |
| H2 | `## 标题` / `### 标题` / `（一）标题` / `1.1 标题` | 红橙色小节标题 |
| H3 | `### 标题` / `#### 标题` / `【标题】` | 红色段落标题 |
| H4 | `#### 标题` / `##### 标题` | 纯文字 |
| 段落 | 普通文字 | 行高 1.9 |
| 加粗 | `**文字**` | 红橙色粗体 |
...
| 内联代码 | `` `代码` `` | 浅灰底 + 红色字 |
| 链接 | `[文字](url)` | 红橙色下划线 |
| 图片 | `![alt](url)` | 居中圆角，说明性 alt 自动生成灰色图注 |
| 无序列表 | `- 项` / `* 项` | 支持套嵌缩进 |
| 有序列表 | `1. 项` | 支持套嵌缩进 |
| 任务列表 | `- [ ] 待办` / `- [x] 已完成` | 复选框 |
| 引用 | `> 引用` | 浅灰底 + 红橙色左边框 |
...
「摸鱼绿」「红白」「摸鱼票据」「橄榄手记」基于 [843645440/wechat-skill](https://github.com/843645440/wechat-skill) 的公开主题设计进行 Web 编辑器适配。原主题项目采用 AGPL-3.0-or-later；本仓库加入这些适配后同样以 AGPL-3.0-or-later 发布，详见 `LICENSE` 与 `THIRD_PARTY_NOTICES.md`。
